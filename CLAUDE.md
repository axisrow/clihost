...
`ao` (agent-orchestrator) is a **Go binary compiled from source** inside a dedicated `golang:1.25-bookworm` build-stage — it is **not** a prebuilt download and **not** an npm package (the public npm `@aoagents/ao` is stuck on the old 0.9.5 Node scheme; no Go prebuilts are published). Both the x86_64 issue (#76) and the arm64 issue (#77) are closed by **this single go-build path**: `ao`'s only native dependency is the pure-Go `modernc.org/sqlite` driver, so `CGO_ENABLED=0 GOOS=linux GOARCH=$TARGETARCH go build -trimpath -ldflags='-s -w' -o /out/ao ./cmd/ao` (run in the `backend/` module) produces a static binary identically for every architecture — no arch branches, no gcc/musl.
