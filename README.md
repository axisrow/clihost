...
Контракт обязательности, startup-проверок и поведения после смерти процесса зафиксирован в [`docs/service-lifecycle.md`](docs/service-lifecycle.md). `entrypoint.sh` не является supervisor и не перезапускает фоновые сервисы.
